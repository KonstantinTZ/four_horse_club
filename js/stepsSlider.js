class StepsSlider {
    constructor(slider, {autoplay = false, inFrame = 1, offset = 1} = {}) {
        // элемент div.steps-carousel
        this.slider = slider;
        ...
        this.allItems = slider.querySelectorAll('.steps-carousel-item');
        ...
        this.wrapper = slider.querySelector('.steps-carousel-slides');
        // кнопка «вперед»
        this.nextButton = slider.querySelector('.steps-carousel-next');
        // кнопка «назад»
        this.prevButton = slider.querySelector('.steps-carousel-prev');
    ...
    init() {
        ...
        this.buttonDisablerIndicators = this.buttonDisabler(this.frameIndex + 1)
        // если всего 10 элементов, то ширина одного элемента составляет 1/10
        // ширины контейнера .steps-carousel-slides, то есть 100/10 = 10%
    ...
    move() {
        ...
        this.buttonDisabler(this.frameIndex + 1)// меняет число на индикаторе
    ...
    dots() {
        ...
        ol.classList.add('steps-carousel-indicators');
        const children = [];
    ...
    //дизаблит кнопку по условию
    buttonDisabler (index) {
        let currentFrame = index
        
        const totalFrames = this.frameCount
        
        if (currentFrame === totalFrames){
            document.getElementById('steps-carousel-next').disabled = true
        } else {
            document.getElementById('steps-carousel-next').disabled = false
        }

        if (currentFrame === 1){ 
            document.getElementById('steps-carousel-prev').disabled = true
        } else {
            document.getElementById('steps-carousel-prev').disabled = false
        }
    }
